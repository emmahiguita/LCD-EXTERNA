...
import com.limelight.ui.SmartDisplayOverlay;
...
public class Game extends Activity implements SurfaceHolder.Callback,
    ...
    private SmartDisplayOverlay smartOverlay;
    private boolean panZoomMode = false;   // true while "MOVER" (free viewport) is active
    private int userOrientationMode = 0;   // 0=AUTO, 1=VERTICAL, 2=HORIZONTAL (from overlay)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Pan/zoom transforms go on the wrapper FrameLayout, never on the SurfaceView itself.
        streamView.setTransformTarget(findViewById(R.id.streamViewWrapper));
        ...
        StreamConfiguration config = new StreamConfiguration.Builder()
                ...
                // 1024 (not the 1392 LAN default) so each video packet fits inside the
                // Tailscale/WireGuard tunnel MTU of 1280 without IP fragmentation. Over mobile
                // data a single lost fragment would drop the whole frame → stutter/lag.
        ...
        // ── SmartDisplay Overlay: FAB + keyboard floating above the stream ──
        // Must be created AFTER conn so keyboard input injection works.
        // The DecorView always wraps activity content in a FrameLayout (android.R.id.content).
        try {
            View contentFrame = getWindow().getDecorView().findViewById(android.R.id.content);
            if (contentFrame instanceof FrameLayout) {
                smartOverlay = new SmartDisplayOverlay(this, (FrameLayout) contentFrame, conn);
                wireSmartOverlay();
            }
        } catch (Exception e) {
            // Overlay is a non-critical feature — never crash the stream over it
            android.util.Log.e("SmartDisplay", "Overlay init failed", e);
        }
    ...
    /** Connects the SmartDisplay overlay actions to real behaviour on the stream. */
    private void wireSmartOverlay() {
        if (smartOverlay == null) {
            return;
        }

        // Live zoom factor → overlay badge
        streamView.setPanZoomListener(scale ->
                runOnUiThread(() -> {
                    if (smartOverlay != null) {
                        smartOverlay.updateZoom(scale);
                    }
                }));

        smartOverlay.setListener(new SmartDisplayOverlay.Listener() {
            @Override
            public void onPanModeChanged(boolean enabled) {
                panZoomMode = enabled;
                // Turning off MOVER mode no longer snaps back to 1:1 automatically.
                // The viewport stays locked in the active zoom/pan position.
            }

            @Override
            public void onZoomReset() {
                streamView.resetPanZoom();
            }

            @Override
            public void onOrientationModeChanged(int mode) {
                userOrientationMode = mode;
    ...
    /** Apply the user's chosen orientation mode (0=AUTO/sensor, 1=VERTICAL, 2=HORIZONTAL). */
    private void applyOrientationMode(int mode) {
        switch (mode) {
            case 1: // VERTICAL
            ...
            case 2: // HORIZONTAL
                ...
                break;
            default: // AUTO — free sensor rotation
    ...
    private void setPreferredOrientationForCurrentDisplay() {
        // SmartDisplay AI: respect the user's chosen orientation (VERTICAL/HORIZONTAL);
        // in AUTO, allow the sensor to dictate portrait or landscape freely.
        applyOrientationMode(userOrientationMode);
    }
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        // Notify SmartDisplay overlay (restores FAB position, preserves keyboard/zoom state)
        if (smartOverlay != null) {
            smartOverlay.onConfigurationChanged(newConfig);
        }
    ...
    private float[] mapTouchToVideoCoordinates(float x, float y) {
        if (streamView == null) {
            return new float[] { x, y };
        }
        float scale = streamView.getCurrentScale();
        float transX = streamView.getCurrentTranslationX();
        float transY = streamView.getCurrentTranslationY();
        if (scale <= 1.0f && transX == 0f && transY == 0f) {
            return new float[] { x, y };
        }
        float pX = streamView.getWidth() / 2.0f;
        float pY = streamView.getHeight() / 2.0f;
        float mappedX = (x - pX - transX) / scale + pX;
        float mappedY = (y - pY - transY) / scale + pY;
    ...
    private float[] getStreamViewRelativeNormalizedXY(View view, MotionEvent event, int pointerIndex) {
        ...
        float[] mapped = mapTouchToVideoCoordinates(normalizedX, normalizedY);
        normalizedX = mapped[0];
        normalizedY = mapped[1];
    ...
    private boolean handleMotionEvent(View view, MotionEvent event) {
                 ...
                 eventSource == InputDevice.SOURCE_MOUSE_RELATIVE)
        {
            ...
            else
            {
                ...
                float[] mapped = mapTouchToVideoCoordinates(event.getX(actionIndex) + xOffset, event.getY(actionIndex) + yOffset);
                int eventX = (int) mapped[0];
                int eventY = (int) mapped[1];
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (actionIndex == 0 && event.getPointerCount() > 1 && !context.isCancelled()) {
                        ...
                        float[] mappedSec = mapTouchToVideoCoordinates(event.getX(1) + xOffset, event.getY(1) + yOffset);
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    for (int i = 0; i < event.getHistorySize(); i++) {
                        for (TouchContext aTouchContextMap : touchContextMap) {
                            ...
                            {
                                float[] mappedHist = mapTouchToVideoCoordinates(
                                        event.getHistoricalX(aTouchContextMap.getActionIndex(), i) + xOffset,
                                        event.getHistoricalY(aTouchContextMap.getActionIndex(), i) + yOffset
                                );
                    ...
                    for (TouchContext aTouchContextMap : touchContextMap) {
                        if (aTouchContextMap.getActionIndex() < event.getPointerCount())
                        {
                            float[] mappedCurr = mapTouchToVideoCoordinates(
                                    event.getX(aTouchContextMap.getActionIndex()) + xOffset,
                                    event.getY(aTouchContextMap.getActionIndex()) + yOffset
                            );
                            aTouchContextMap.touchMoveEvent(
                                    (int)mappedCurr[0],
                                    (int)mappedCurr[1],
    ...
    private void updateMousePosition(View touchedView, MotionEvent event) {
        ...
        float[] mapped = mapTouchToVideoCoordinates(eventX, eventY);
        eventX = mapped[0];
        eventY = mapped[1];
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        // In MOVER (free viewport) mode, finger touches pan/zoom the video instead of
        // driving the remote mouse. Deltas are origin-independent, so it works even
        // though these events are relative to the background touch view.
        if (panZoomMode) {
            return streamView.handlePanZoomTouch(event);
        }
