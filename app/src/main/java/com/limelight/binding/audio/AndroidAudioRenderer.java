...
public class AndroidAudioRenderer implements AudioRenderer {
    ...
    // SmartDisplay: silencia la salida del audio del PC en el dispositivo sin
    // tocar el pipeline de Moonlight. Se usa al activar la voz (walkie-talkie)
    // para evitar que el altavoz alimente el micrófono y reenvíe eco al PC.
    ...
    public void setMuted(boolean muted) {
        ...
        AudioTrack t = track;
        if (t != null) {
            try { t.setVolume(muted ? 0.0f : 1.0f); } catch (Exception ignored) {}
        }
    }
    ...
    public int setup(MoonBridge.AudioConfiguration audioConfiguration, int sampleRate, int samplesPerFrame) {
        ...
        // Respetar el estado de mute si la voz ya estaba activa (p. ej. tras
        // una reconexión que recrea el track).
        if (muted) {
            try { track.setVolume(0.0f); } catch (Exception ignored) {}
        }
    ...
    public void playDecodedAudio(short[] audioData) {
        // [FIX] Tolerancia aumentada a 250ms para evitar cortes y pérdida de audio por jitter en la red, 
        // manteniendo un buen equilibrio de latencia y fluidez.
        if (MoonBridge.getPendingAudioDuration() < 250) {
            track.write(audioData, 0, audioData.length);
        }
        else {
            LimeLog.info("Too much pending audio data: " + MoonBridge.getPendingAudioDuration() +" ms (Drop)");
        }
    }
