...
import com.limelight.smartdisplay.profile.ActiveProfile;
...
public class RelativeTouchContext implements TouchContext {
    ...
    public void touchUpEvent(int eventX, int eventY, long eventTime)
    {
        ...
        {
            if (buttonIndex == MouseButtonPacket.BUTTON_RIGHT && onRightClickListener != null) {
                // Notificar el click derecho (usando las coordenadas actuales de touch, aunque sea relativo
                // servirá como centro aproximado del foco táctil)
                onRightClickListener.onRightClick(lastTouchX, lastTouchY);
            } else if (buttonIndex == MouseButtonPacket.BUTTON_LEFT && onLeftClickListener != null) {
                onLeftClickListener.onLeftClick(lastTouchX, lastTouchY);
            }
    ...
    public boolean touchMoveEvent(int eventX, int eventY, long eventTime)
    {
        ...
        if (eventX != lastTouchX || eventY != lastTouchY)
        {
            ...
            if (actionIndex == 0) {
                ...
                if (pointerCount == 2) {
                    if (confirmedScroll) {
                        // SmartDisplay Touch Engine 2.0: sensibilidad de scroll por perfil
                        // (default 1.0 = comportamiento original). naturalScroll invierte el eje.
                        float scrollSens = ActiveProfile.getScrollSensitivity();
                        int scroll = (int) Math.round((double) deltaY * SCROLL_SPEED_FACTOR * scrollSens);
                        if (ActiveProfile.isNaturalScroll()) {
                            scroll = -scroll;
                        }
                        conn.sendMouseHighResScroll((short) scroll);
                    }
                } else {
                    // SmartDisplay Touch Engine 2.0: sensibilidad del puntero por perfil
                    // (default 1.0 = comportamiento original).
                    float sens = ActiveProfile.getPointerSensitivity();
                    int sDeltaX = (int) Math.round((double) deltaX * sens);
                    int sDeltaY = (int) Math.round((double) deltaY * sens);
                    if (prefConfig.absoluteMouseMode) {
                        ...
                        conn.sendMouseMove((short) sDeltaX, (short) sDeltaY);
                    }
                }
