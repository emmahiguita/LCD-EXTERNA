                                        ...
                                        if (details.state == ComputerDetails.State.ONLINE && details.pairState == PairingManager.PairState.PAIRED) {
                                            ...
                                            if (app != null) {
                                                ...
                                                new Thread(new Runnable() {
                                                    ...
                                                    public void run() {
                                                        try {
                                                            // Captura local del binder: evita NPE si el servicio se
                                                            // desconecta entre el sondeo y el arranque de la app.
                                                            ...
                                                            if (binder == null) {
                                                                ...
                                                                runOnUiThread(new Runnable() {
                                                                    ...
                                                                    public void run() {
                                                                        Dialog.displayDialog(ShortcutTrampoline.this,
                                                                                getResources().getString(R.string.conn_error_title),
                                                                                "El servicio no está listo. Vuelve a intentarlo.", true);
                                                                    }
                                                                });
                                                                return;
                                                            }
                                                            ...
                                                            if (appList != null && !appList.isEmpty()) {
                                                                // Opcional: nombre de app objetivo (p. ej. "steam").
                                                                // Si no se indica, se busca el escritorio remoto.
                                                                final String requestedName = getIntent().getStringExtra("TARGET_APP_NAME");
                                                                NvApp targetApp = null;
                                                                for (NvApp a : appList) {
                                                                    String n = a.getAppName().toLowerCase();
                                                                    if (requestedName != null && !requestedName.isEmpty()) {
                                                                        if (n.contains(requestedName.toLowerCase())) {
                                                                            targetApp = a;
                                                                            break;
                                                                        }
                                                                ...
                                                                if (targetApp == null && (requestedName == null || requestedName.isEmpty())) {
                                                                    // Solo caemos al primero cuando buscábamos el escritorio.
                                                                    targetApp = appList.get(0);
                                                                }
                                                                if (targetApp == null) {
                                                                    // Se pidió una app concreta (p. ej. Steam) y no existe.
                                                                    final String missing = requestedName;
                                                                    ShortcutTrampoline.isTrampolining = false;
                                                                    runOnUiThread(new Runnable() {
                                                                        @Override
                                                                        public void run() {
                                                                            Dialog.displayDialog(ShortcutTrampoline.this, "Error",
                                                                                    "No se encontró la aplicación \"" + missing + "\" en el PC.", true);
                                                                        }
                                                                    });
                                                                    return;
                                                                }
                                                                ...
                                                                final Intent startIntent = ServerHelper.createStartIntent(ShortcutTrampoline.this, targetApp, details, binder);
                                            ...
                                            if (getIntent().getBooleanExtra("AUTO_CONNECT", false)) {
                                                ...
                                                // PIN fijo "9999" (red local). Debe ingresarse el mismo PIN en Sunshine.
                                                final String pinStr = com.limelight.nvstream.http.PairingManager.generatePinString();
                                                Log.i("ShortcutTrampoline", "Auto-pairing started");
                                                    ...
                                                    public void run() {
                                                        ...
                                                        for (int attempt = 1; attempt <= 3; attempt++) {
                                                            try {
                                                                ...
                                                                // SmartDisplay: auto-enviar el PIN a Sunshine (sin abrir la web)
                                                                com.limelight.smartdisplay.host.SunshinePairHelper.submitPinAsync(
                                                                        ShortcutTrampoline.this, pairAddress.address, pinStr, "SmartDisplay");
                                                                ...
                                                                if (pairState == PairingManager.PairState.PAIRED) {
                                                                    ...
                                                                    // PIN fijo "9999": debe ingresarse exactamente ese valor en Sunshine.
                                                                    lastError = "PIN rechazado - ingresa " + pinStr + " en Sunshine";
                                                                    Thread.sleep(5000);
                                                                } else {
