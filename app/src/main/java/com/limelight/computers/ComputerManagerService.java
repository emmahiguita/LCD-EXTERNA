...
public class ComputerManagerService extends Service {
                    ...
                    try {
                        ...
                        synchronized (tuple.networkLock) {
                            ...
                            if (!runPoll(tuple.computer, false, offlineCount)) {
                                ...
                                offlineCount++;
                                // Reset backoff: si cae, volvemos a sondeo rápido
                                tuple.consecutiveSuccesses = 0;
                            } else {
                                tuple.lastSuccessfulPollMs = SystemClock.elapsedRealtime();
                                offlineCount = 0;
                                tuple.consecutiveSuccesses++;
                            }
                        }

                        // Backoff adaptativo: rápido cuando es nuevo/inestable, lento cuando está estable.
                        // •  0–4  éxitos → 1500 ms  (comportamiento original)
                        // •  5–14 éxitos → 4000 ms  (moderado, ahorra batería/red)
                        // • 15+   éxitos → 8000 ms  (estable, mínimo impacto)
                        final int pollInterval;
                        if (tuple.consecutiveSuccesses >= 15) {
                            pollInterval = 8000;
                        } else if (tuple.consecutiveSuccesses >= 5) {
                            pollInterval = 4000;
                        } else {
                            pollInterval = SERVERINFO_POLLING_PERIOD_MS;
                        }
    ...
    public class ComputerManagerBinder extends Binder {
        ...
        public void stopPolling() {
            pollingActive = false;
            ComputerManagerService.this.listener = null;
    ...
    public boolean onUnbind(Intent intent) {
        ...
        // Cancel all active network requests immediately
        com.limelight.nvstream.http.NvHTTP.cancelAllActiveCalls();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null && "com.limelight.computers.RELOAD".equals(intent.getAction())) {
            LimeLog.info("Reloading computers from database...");
            if (getLocalDatabaseReference()) {
                synchronized (pollingTuples) {
                    for (ComputerDetails computer : dbManager.getAllComputers()) {
                        addTuple(computer);
...
class PollingTuple {
    ...
    /** Contador de polls consecutivos exitosos. Usado para el backoff adaptativo. */
