...
public class ComputerDatabaseManager {
    ...
    private void initializeDb(Context c) {
        ...
        List<ComputerDetails> oldComputers = LegacyDatabaseMigrator.migrateV1(c);
        for (ComputerDetails computer : oldComputers) {
            updateComputer(computer);
        }
        oldComputers = LegacyDatabaseMigrator.migrateV2(c);
        for (ComputerDetails computer : oldComputers) {
            updateComputer(computer);
        }
        oldComputers = LegacyDatabaseMigrator.migrateV3(c);
        for (ComputerDetails computer : oldComputers) {
            updateComputer(computer);
        }
    }
